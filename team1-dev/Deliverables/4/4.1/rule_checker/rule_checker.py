...
class rule_checker:
    ...
    def check_history(self, boards, initial_stone):
        """
        Verifies that board history is valid
        :return: Bool
        """
        if len(boards) == 1:
            return empty_board(boards[0])
        ...
        if len(boards) == 2:
            ...
            return self.valid_between_two_boards(last_player,
                                                 [last_played_point(boards, last_player), boards], initial_stone)
